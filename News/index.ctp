...
<div class="container">
	<div class="row">
		<div class="col-lg-8">
			<h1 style="font-family: 'Concert One', sans-serif;margin-bottom:20px;    text-align: center;"><?= before_display($news['News']['title']) ?></h1>

			<p class="lead"><?= $news['News']['content'] ?></p>
			<button style="border-radius:0px;border:none;background-color:#337ab7" id="<?= $news['News']['id'] ?>"
					type="button"
					class="btn btn-primary pull-right like<?= ($news['News']['liked']) ? ' active' : '' ?>"
			<?= (!$Permissions->can('LIKE_NEWS')) ? ' disabled' : '' ?>><?= $news['News']['count_likes'] ?> <i
				class="fa fa-thumbs-up"></i></button><br>
		...
		<div class="col-xs-4">
			<div class="well">
				<h4 style="font-family: 'Concert One', sans-serif;margin-bottom:20px;text-align:center"><?= $Lang->
					get('GLOBAL__INFORMATIONS') ?></h4>
				<p style="font-family: 'Righteous', cursive;color:#337ab7"><b style="color:black"><?= $Lang->
